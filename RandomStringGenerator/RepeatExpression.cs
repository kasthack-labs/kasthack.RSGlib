...
namespace RandomStringGenerator
{
    public class RepeatExpression : IExpression
    {
        ...
        public char[] GetChars() { return GetChars(Generators.Random.Next(this._min, this._max)); }
        public byte[] GetEncodingBytes(Encoding enc) { return GetEncodingBytes(enc, Generators.Random.Next(this._min, this._max)); }
        public System.Collections.Generic.IEnumerable<byte[]> EnumAsciiBuffers() {
            return Enumerable.Range(0, Generators.Random.Next(this._min, this._max)).
                              SelectMany(a => Expressions.SelectMany(b => b.EnumAsciiBuffers())).
                              ToArray();
        }
