...
namespace RandomStringGenerator
{
    public class StaticASCIIStringExpression : IExpression
    {
        ...
        public string GetString() { return this._enc.GetString(this._buf); }
        ...
        public unsafe void ComputeStringLength(ref int* outputdata) { *outputdata++ = this._buf.Length; }
        ...
        public unsafe void GetAsciiInsert(ref int* size, ref char* outputBuffer) {
            fixed ( byte* tmpbuf = this._buf ) {
                ...
                var end = tmpbuf + *size++;
                do
                    *outputBuffer++ = (char) *start++; while( start < end );
            }
        }
